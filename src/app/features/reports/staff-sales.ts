    ...
    <div class="space-y-4">
      ...
      <div class="grid gap-4 xl:grid-cols-2">
        <ui-card
          ...
          subheading="Ranked by attributed value"
          icon="briefcase"
          [padded]="false"
        >
          <div class="p-4">
        ...
        <ui-card
          heading="Sales by referral source"
          subheading="Where the business comes from"
          icon="compass"
          [padded]="false"
        >
          <div class="p-4">
            @if (loading()) {
              ...
              <ui-columns
                ...
                [height]="230"
                color="var(--viz-3)"
                ariaLabel="Sales by referral source"
              />
            } @else {
          ...
          <ul class="space-y-3 border-t border-line p-4">
            @for (row of filteredReferrals(); track row.id; let i = $index) {
              <li class="stagger flex items-center gap-3" [style]="'--i:' + i">
                <span
                  class="grid size-7 shrink-0 place-items-center rounded-lg bg-surface-2 text-muted"
                >
...
export class StaffSalesReportPage {
  ...
  private employeeRows(): Record<string, unknown>[] {
    return this.filteredEmployees().map((row) => ({
      Type: 'Employee',
      Name: row.name,
  ...
  private referralRows(): Record<string, unknown>[] {
    return this.filteredReferrals().map((row) => ({
      Type: 'Referral',
      Name: row.name,
      Invoices: row.invoiceCount,
      Amount: row.amount,
  ...
  protected exportCsv(): void {
    downloadCsv('staff-and-referral-sales', [...this.employeeRows(), ...this.referralRows()]);
  }
  ...
  protected printPdf(): void {
    this.print.report({
      ...
      filename: 'staff-and-referral-sales',
      filters: [{ label: 'Search', value: this.search() || 'Everyone' }],
      summary: [
        { label: 'Total attributed', value: currency(this.totalSales()) },
        { label: 'Active sellers', value: String(this.employees().length) },
        { label: 'Referral sources', value: String(this.referrals().length) },
      ],
      sections: [
        {
          heading: 'By employee',
          ...
          totals: {
            Invoices: sum(this.filteredEmployees(), (row) => row.invoiceCount),
            Amount: sum(this.filteredEmployees(), (row) => row.amount),
          },
          emptyMessage: 'No employee sales in this window.',
        },
        {
          heading: 'By referral source',
          rows: this.referralRows(),
          totals: {
            Invoices: sum(this.filteredReferrals(), (row) => row.invoiceCount),
            Amount: sum(this.filteredReferrals(), (row) => row.amount),
          },
          emptyMessage: 'No referred sales in this window.',
        },
      ],
    });
  }
